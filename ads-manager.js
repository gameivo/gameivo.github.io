 ...
 * ✅ إصلاح Popunder للعمل مرة واحدة فقط (حتى بعد التحديث)
...
class AdsManager {
  ...
  // === التصحيح الرئيسي: إصلاح دالة تحميل الإعلان وتعديل التجاوب ===
  loadSingleAd(container, ad, containerId) {
    ...
    // ✅ إصلاح الموبايل: إضافة max-width و overflow
    adDiv.style.cssText = `max-width:100%; overflow:hidden; position:relative;`;
    
    adDiv.innerHTML = `
      <div class="ad-label">Advertisement</div>
      <div id="banner-${uniqueId}" style="text-align:center;min-height:${ad.config?.height || 90}px;background:transparent;width:100%;max-width:100%;overflow:hidden;"></div>
    `;
  ...
  loadSidebarAd(container, ad) {
    ...
    // ✅ إصلاح الموبايل للجانب
    adDiv.style.cssText = `max-width:100%; overflow:hidden; position:relative;`;
    
    adDiv.innerHTML = `
      <div class="ad-label">Advertisement</div>
      <div id="sidebar-${uniqueId}" style="text-align:center;min-height:${ad.config?.height || 300}px;background:transparent;width:100%;max-width:100%;overflow:hidden;"></div>
    `;
  ...
  // === 13. تحميل Popunder - مُصلح ✅ (بذاكرة تخزين دائمة) ===
  loadPopunder() {
    if (!this.config.popunder?.enabled) return;
    
    const maxPerSession = this.config.popunder.maxPerSession || 1;
    
    // ✅ إصلاح: التحقق من العداد في الذاكرة لضمان عدم التكرار عند التحديث
    if (this.sessionData.popunderCount >= maxPerSession) {
        console.log(`⚠️ Popunder limit reached: ${this.sessionData.popunderCount}/${maxPerSession}`);
        return;
    }
    ...
    setTimeout(() => {
      ...
      // ✅ تحديث وحفظ البيانات في localStorage لتبقى حتى بعد الـ Refresh
      this.sessionData.popunderCount = (this.sessionData.popunderCount || 0) + 1;
      this.sessionData.popunderShown = true;
      this.saveSessionData();
      
      console.log(`📊 Popunder shown: ${this.sessionData.popunderCount}/${maxPerSession}`);
  ...
  fixAdContainers() {
    ...
    containers.forEach(containerId => {
      ...
      if (!container) {
        ...
        container.style.cssText = `min-height: 50px; margin: 20px 0; position: relative;`;
        
        // منطق الإدراج (نفس كودك الأصلي)
        if (containerId.includes('above')) {
           const game = document.querySelector('.game-container');
           if (game) game.prepend(container);
        } else {
  ...
  showFallbackAds() {
    ...
    ['ad-above-iframe', 'ad-sidebar'].forEach(id => {
      const el = document.getElementById(id);
      if (el) el.innerHTML = `<div class="ad-banner"><div class="ad-label">Ads</div><p>Support us by disabling AdBlock</p></div>`;
    });
  }
  ...
  showFallbackInContainer(container) {
    if (!container) return;
    container.innerHTML = `<div class="ad-banner"><div class="ad-label">Ads</div><p>Support us by allowing ads</p></div>`;
  }

  // === 19. إدارة الجلسة (مُعدلة للاستخدام الدائم localStorage) ===
  getSessionData() {
    try {
      // ✅ تم التغيير إلى localStorage لحل مشكلة التحديث
      const data = localStorage.getItem('adsSessionData_v2');
      ...
      return { popunderShown: false, popunderCount: 0, smartlinkOpened: false, adsLoaded: 0, sessionId: Date.now() };
    }
  }

  saveSessionData() {
    try {
      localStorage.setItem('adsSessionData_v2', JSON.stringify(this.sessionData));
    } catch (error) {}
  }
  ...
  filterUnityErrors() {
    ...
    console.error = function(...args) {
      if (args[0] && typeof args[0] === 'string' && (args[0].includes('script') || args[0].includes('is missing'))) return;
...
// === تشغيل تلقائي وتنسيقات CSS الموبايل ===
document.addEventListener('DOMContentLoaded', () => {
  ...
  style.textContent = `
    ...
    /* ✅ إصلاحات الموبايل الجوهرية لضمان عدم خروج الإعلان عن الشاشة */
    .ad-banner iframe, .ad-banner ins, .ad-banner div {
    ...
    @media (max-width: 768px) {
      .ad-banner { padding: 5px; margin: 10px 0; max-width: 100vw !important; overflow: hidden; }
      .ad-sidebar { position: static; width: 100%; }
    }
    
    body.adblock-blocked { overflow: hidden !important; }
    body.adblock-blocked > *:not(#adblock-block-overlay) { pointer-events: none !important; opacity: 0.3; filter: blur(2px); }
  `;
